        ...
        <div class="main">
            <div class="title">
                ...
                <div class="success-tips">
                    <p class="tips1">您已成功购买 {{ num }} 门课程！</p>
                    <p class="tips2">你还可以加入QQ群 <span>1466446026</span> 学习交流</p>
                </div>
            </div>
            <div class="order-info">
                <p class="info1"><b>付款时间：</b><span>{{ time }}</span></p>
                <p class="info2"><b>付款金额：</b><span>￥{{ real_price }}元</span></p>
                <p class="info3" v-for="cousrse in course_list"><b>课程信息：</b><span>{{ cousrse.name }}</span></p>
            </div>
    ...
    methods: {
        aliPayResult() {
            this.$axios.get(this.$settings.HOST + "payments/result/" + location.search).then(res => {
                ...
                this.num = res.data.course_list.length;
                this.time = res.data.pay_time;
